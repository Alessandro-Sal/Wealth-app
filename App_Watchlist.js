...
function getWatchlistData() {
  return getFromCache('WATCHLIST', function() {
    try {
...
function addToWatchlist(ticker) {
  ...
  invalidateWatchlistCache(); // Invalidate cache after addition
...
function removeFromWatchlist(row) {
  ...
  if (row && row > 1) {
    sheet.deleteRow(row);
    invalidateWatchlistCache(); // Invalidate cache after deletion
  } else {
